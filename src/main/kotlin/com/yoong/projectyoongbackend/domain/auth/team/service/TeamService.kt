...
class TeamService(
    ...
    fun createTeam(teamRequest: TeamRequest, memberId: Long): DefaultResponse {
        ...
        val member = memberRepository.findByIdOrNull(memberId)?: throw RuntimeException("맴버가 존재 하지 않습니다")

        if(member.team!!.name != "DUMMY_TEAM") throw RuntimeException("팀에서 탈퇴 먼저 해주세요")
    ...
    fun updateTeam(teamRequest: TeamRequest, teamId: Long): DefaultResponse {
        ...
        if(teamId == 1L) throw RuntimeException("임시 팀은 선택 할 수 없습니다")

        val team = teamRepository.findByIdOrNull(teamId) ?: throw RuntimeException("팀이 존재 하지 않습니다")
    ...
    fun deleteTeam(teamId: Long, leaderId: Long): DefaultResponse {
        ...
        val leader = memberRepository.findByIdOrNull(leaderId) ?: throw RuntimeException("리더가 존재 하지 않습니다")

        if(1 < team.peopleCount) throw RuntimeException("팀 내 맴버가 남아 있습니다 확인 부탁 드립니다")
    ...
    fun grantManagerPrivileges(userId: Long, teamId: Long): DefaultResponse {
        ...
        val member = memberRepository.findByIdOrNull(userId) ?: throw RuntimeException("맴버가 존재 하지 않습니다")

        val team = teamRepository.findByIdOrNull(teamId) ?: throw RuntimeException("팀이 존재 하지 않습니다")

        if(member.team!!.id != teamId) throw RuntimeException("다른 팀 맴버 입니다")

        if(member.position != Position.MEMBER) throw RuntimeException("이미 다른 보직을 맡고 있습니다")
    ...
    fun inviteTeam(teamId: Long, userId: Long): DefaultResponse {
        ...
        if(member.team!!.id != 1L) throw RuntimeException("다른 팀 소속 입니다")
    ...
    fun firedTeam(teamId: Long, userId: Long): DefaultResponse {
        ...
        if(member.team!!.id != teamId) throw RuntimeException("다른 팀 소속 입니다")
